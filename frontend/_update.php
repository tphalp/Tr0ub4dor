...
if ((isset($_POST['step2'])) && ($_POST['step2'] == "true"))
{
	...
	else
	{
		...
		if ($dberror == 1)
		{
			echo "<p>Can't access the database <b>".$_POST['dbname']."</b>. Please go to the previous step and check the database name and also the mysql authorization variables in the file <b>lib/config.php</b>.</p>\n";
			echo "<a href=\"javascript:history.back()\">go to previous step</a>\n";
		}
	}
	echo "</body>\n</html>";
		...
		if (mysql_select_db($_POST['dbname'], $conn))
		{
			...
			if ($crypt_pw == $db_pw)
			{
				...
				if ($conn_new = mysql_connect(DB_HOST, DB_USER, DB_PASS, TRUE))
				{
					...
					{
						echo "<p>Can't connect to the database <b>".DB_NAME."</b> defined in the configuration file <b>lib/config.php</b>. Please check and try again.</p>\n";
						echo "<a href=\"javascript:history.back()\">go to previous step</a>\n";
					}
				}
			}
