<?php
  ...
  if(isset($_FILES['csvfile']) && !empty($_FILES['csvfile']['tmp_name'])) {
    ...
    if(is_uploaded_file($_FILES['csvfile']['tmp_name'])) {
      // upload successful
      copy($_FILES['csvfile']['tmp_name'], TMP_PATH . TMP_IMPORT_FILE);
...
OUT;
        ...
        while ($data = fgetcsv ($fd, 4096, ";")) {
          ...
          if ($linecounter <= 3) {  //only the first 4 lines
            $out__ .= '<tr class="even">';
        ...
        $out__ .= '<span class="note">Note: Only the first three lines of your file are shown here, <br />though the entire file will be imported.</span><br />';
      }
