...
class Data_MySQLi {
	...
	public function out_array($query, $out_type = "object", $array_type = MYSQLI_ASSOC) {
	//---------------------------------------------------
	...
	//---------------------------------------------------
		...
		if (!$db_link) {
			//die("Unable to connect to the database: " . $db_link->connect_error());
			//exit;
		...
		if ($out_type == "object") {
			...
			while($row = $rs->fetch_array($array_type)) {
				array_push($arr, $row);
			}
		}
